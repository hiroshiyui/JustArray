...
- [x] Add reverse lookup (character → code) for learning Array codes
- [ ] Add clipboard history (last N items) shown in candidate bar when Idle

## Keyboard UI

- [x] Add visual feedback when composing buffer is full (5th key silently ignored)
- [x] Show uppercase key labels when Shift/CapsLock is active in English mode
